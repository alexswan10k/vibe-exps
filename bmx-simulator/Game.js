class Game {
    ...
    loadTrack(index) {
        ...
        this.collisionData = this.collisionCtx.getImageData(0, 0, this.track.logicalWidth, this.track.logicalHeight);
        ...
        this.world = {
            ...
            groundAt: (x, y) => (this.track.heightField ? this.track.heightField.sample(x, y) : 0)
        };
    ...
    buildMinimapBg() {
        ...
        const mw = 168;
        const mh = Math.round(mw * this.track.logicalHeight / this.track.logicalWidth);
        ...
        switch (type) {
            case 'skid':
                this.particles.spawn(d.x, d.y, {
                    ...
                    size: 5, sizeEnd: 11, life: 0.55,
                    color: 'rgba(160,160,165,0.7)', drag: 3
                });
                break;
            case 'hop':
            ...
            case 'bump':
                ...
                break;
            case 'land': {
                this.particles.burst(bike.x, bike.y, 10, 90, { size: 4, sizeEnd: 9, life: 0.5, color: '#9c8f7c', drag: 4 });
                if (bike.isPlayer) {
                    this.audio.land();
            ...
            case 'trick':
                this.addFloatText(bike.isPlayer ? '360 AIR! +BOOST' : 'TRICK!', bike.x, bike.y - 24, '#ffeb3b', bike.isPlayer ? 22 : 14);
                if (bike.isPlayer) this.audio.trick();
                break;
            case 'crash':
                this.particles.burst(bike.x, bike.y, 16, 130, { size: 4, sizeEnd: 10, life: 0.7, color: '#b0a794', drag: 3 });
                if (bike.isPlayer) {
                    this.audio.crash();
                    this.shake = 12;
                } else {
                    this.particles.burst(bike.x, bike.y, 4, 60, { size: 3, life: 0.4, color: '#888', drag: 4 });
                }
                break;
        }
    }
    ...
    update(dt) {
        ...
        this.audio.update(dt, this.playerBike ? this.playerBike.speedNorm : 0, this.state === 'racing');
        ...
        if (Input.consume('respawn') && racing && this.playerBike.state !== 'crashed') {
        ...
        if (this.playerBike) {
            ...
            this.playerBike.update(dt, { thrust, turn, hop }, this.world, this.track);
        ...
        for (const ai of this.aiControllers) {
            if (racing) {
                ...
                ai.bike.update(dt, { thrust: 0, turn: 0, hop: false }, this.world, this.track);
            }
        }
    ...
    checkWaypointsAndLaps() {
        ...
        for (const bike of this.bikes) {
            ...
            if (MathUtils.distance(bike.x, bike.y, wp.x, wp.y) < this.track.wpRadius) {
                ...
                if (bike.currentWaypoint === 1 && bike.lastPassedWaypoint === 0) {
                    ...
                    if (!bike.finished && bike.lapsCompleted >= this.track.laps) {
                        bike.finished = true;
    ...
    finishRace() {
        ...
        this.addFloatText('FINISH!', this.playerBike.x, this.playerBike.y - 34, '#ffeb3b', 26, 2);
        setTimeout(() => {
    ...
    buildResults(final = false) {
        ...
        const winnerTime = standings[0] && standings[0].finished ? standings[0].finishTime : null;
        ...
        standings.forEach((bike, i) => {
            ...
            if (bike.finished) {
                time.innerText = winnerTime != null && bike.finishTime > winnerTime
    ...
    updateCamera(dt) {
        ...
        this.camera.zoom += (targetZoom - this.camera.zoom) * (1 - Math.exp(-2.5 * dt));

        const halfW = this.canvas.width / (2 * this.camera.zoom);
        const halfH = this.canvas.height / (2 * this.camera.zoom);
        const lw = this.track.logicalWidth;
        const lh = this.track.logicalHeight;
        this.camera.x = halfW * 2 >= lw ? lw / 2 : MathUtils.clamp(this.camera.x, halfW, lw - halfW);
        this.camera.y = halfH * 2 >= lh ? lh / 2 : MathUtils.clamp(this.camera.y, halfH, lh - halfH);
    ...
    draw() {
        ...
        ctx.save();
        const shakeX = this.shake ? MathUtils.randRange(-this.shake, this.shake) : 0;
        const shakeY = this.shake ? MathUtils.randRange(-this.shake, this.shake) : 0;
        ...
        for (const f of this.floatingTexts) {
            const sx = (f.x - this.camera.x) * this.camera.zoom + this.canvas.width / 2;
            const sy = (f.y - this.camera.y) * this.camera.zoom + this.canvas.height / 2;
    ...
    drawMinimap() {
        ...
        mctx.lineWidth = 1;
        mctx.strokeRect(
            (this.camera.x - halfW) * s, (this.camera.y - halfH) * s,
            halfW * 2 * s, halfH * 2 * s
        );
