class HeightField {
        ...
        rasterStrip(f) {
                ...
                for (let r = r0; r <= r1; r++) {
                        for (let c = c0; c <= c1; c++) {
                                ...
                                const nx =
                                        P[bestI].nx +
                                        (P[bestI + 1].nx - P[bestI].nx) * bestT;
                                const ny =
                                        P[bestI].ny +
                                        (P[bestI + 1].ny - P[bestI].ny) * bestT;
        ...
        rasterRollerBand(f) {
                ...
                const c0 = Math.max(
                        0,
                        Math.floor((Math.min(f.x1, f.x2) - pad) / cs),
                );
                const c1 = Math.min(
                        this.cols - 1,
                        Math.ceil((Math.max(f.x1, f.x2) + pad) / cs),
                );
                const r0 = Math.max(
                        0,
                        Math.floor((Math.min(f.y1, f.y2) - pad) / cs),
                );
                const r1 = Math.min(
                        this.rows - 1,
                        Math.ceil((Math.max(f.y1, f.y2) + pad) / cs),
                );
                for (let r = r0; r <= r1; r++) {
                        for (let c = c0; c <= c1; c++) {
                                const wx = (c + 0.5) * cs;
                                ...
                                const cl = MathUtils.clamp(
                                        Math.abs(su) / f.w,
                                        0,
                                        1,
                                );
                                const v = 0.5 * (1 + Math.cos(Math.PI * cl));
                                // Feather the ends over a wide band: a hard 6px drop at the road
                                // edge reads as a cross-slope spike, and camber steers off
                                // cross-slope, so a roller would yank the bars at its tips.
                                const edgeFade = MathUtils.clamp(
                                        (f.halfAcross - Math.abs(sn)) / 14,
                                        0,
                                        1,
                                );
