...
try {
  for (const Transport of [Net, PollNet]) {
    now = 0;
     ...
     assert.equal(typeof net.vehicleControl, "function");
...
const recipes = await load("client/js/recipes.js");
const locomotiveRecipe = recipes.SHAPED_CLIENT.find((recipe) => recipe.id === "locomotive");
assert.equal(locomotiveRecipe?.out.id, 160);
const netSource = readFileSync(new URL("client/js/net.js", root), "utf8");
assert.match(netSource, /vehicleControl\(throttle\)/);
...
const htmlSource = readFileSync(new URL("client/index.html", root), "utf8");
assert.match(mainSource, /ui\.onPlay = \(name\)/);
assert.match(mainSource, /connectGame\(server, name/);
assert.match(mainSource, /ui\.onCreativeGive/);
assert.match(uiSource, /this\.onPlay\?\.\(n\)/);
assert.match(uiSource, /buildCreativeBrowser/);
assert.match(htmlSource, /id="creative-browser"/);
assert.match(entitiesSource, /let eyeMat = null/);
console.log("PASS: server dispatch, both client cooldown boundaries, TeamPings export, shared coordinate/settings parity, startup wiring, locomotive recipe, creative browser wiring");
